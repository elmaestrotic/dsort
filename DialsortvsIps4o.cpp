/**
 * DialSort vs. IPS4o — Direct Comparison Benchmark
 * ==================================================
 ...
 * and IPS4o (sequential only in this Windows/MinGW version) across all
 * experimental dimensions used in the paper.
 *
 * DEPENDENCY
 * ----------
 * IPS4o is a header-only library.
 *   git clone --recursive https://github.com/ips4o/ips4o
 *   Then compile with: -I./ips4o/include
 *
 * IMPORTANT
 * ---------
 * This version explicitly disables IPS4o parallel support in order to avoid
 * TBB dependency issues on CLion + MinGW under Windows.
 *
 * WHAT IS MEASURED
 * ----------------
 *   1. DialSort            : 2 passes, O(U) memory, NO prefix sum, in-place
 *   2. DialSort-Parallel   : same as above, 8 threads (parallel ingestion)
 *   3. IPS4o (sequential)  : comparison-based, O(n log n), in-place
...
int main() {
    std::cout
            ...
            << "Seed         : " << SEED << "\n"
            #ifdef SKIP_IPS4O
            << "IPS4o        : NOT COMPILED (recompile without -DSKIP_IPS4O)\n"
            #else
            << "IPS4o        : ENABLED (SEQUENTIAL ONLY)\n"
            #endif
            << "================================================================\n\n";
    ...
    std::cout << "TABLE — DialSort vs IPS4o vs std::sort\n";
    std::cout << "Column 'vs std::sort': speedup relative to GCC introsort\n\n";
    print_table_header();

    struct GroupResult {
        Row dialsort, parallel, ips4o_seq, std_sort;
    };
...
#ifndef SKIP_IPS4O
                auto r_ips4o_seq = run_one("IPS4o-seq", dist.name, base, U, fn_ips4o_seq, baseline_ms);
#endif
    ...
    // ── Summary ───────────────────────────────────────────────────────────────
    std::cout << "================================================================\n";
    std::cout << "SUMMARY — DialSort vs IPS4o\n";
    std::cout << "================================================================\n\n";

#ifndef SKIP_IPS4O
    int ds_beats_ips4o = 0, total = 0;
    double sum_ratio = 0;
    ...
    for (const auto& g : results) {
        ...
        ++total;
        const double ratio = g.dialsort.ms / g.ips4o_seq.ms;
        sum_ratio += ratio;
        min_ratio = std::min(min_ratio, ratio);
        max_ratio = std::max(max_ratio, ratio);
        if (ratio < 1.0) ++ds_beats_ips4o;
    }

    std::cout << std::fixed << std::setprecision(3);
    std::cout << "Configurations measured              : " << total << "\n"
              << "DialSort faster than IPS4o-seq       : " << ds_beats_ips4o << " / " << total << "\n"
              << "Avg ratio DialSort / IPS4o-seq        : " << (total > 0 ? sum_ratio / total : 0) << "x\n"
              << "Min ratio DialSort / IPS4o-seq (best) : " << min_ratio << "x\n"
              << "Max ratio DialSort / IPS4o-seq (worst): " << max_ratio << "x\n\n"
              << "NOTE: ratio < 1.0 => DialSort is faster than IPS4o-seq\n"
              << "      ratio > 1.0 => IPS4o-seq is faster than DialSort\n\n";
#else
    std::cout << "IPS4o not compiled. Recompile without -DSKIP_IPS4O to see comparison.\n\n";
#endif
...
#ifndef SKIP_IPS4O
        if (!g.ips4o_seq.skipped && !g.ips4o_seq.correct) { all_ok = false; break; }
#endif
    }
